...
parser.add_argument('--teacher_path',              type=str,   help='path to teacher model (ViT-L) for distillation', default='')
parser.add_argument('--teacher_model',             type=str,   help='teacher dinov2 model name', default='dinov2_vitl14')
parser.add_argument('--distill_weight',            type=float, help='weight for distillation loss', default=0.5)
...
# ===================================================================
def load_teacher(args, gpu):
    """載入 ViT-L teacher model，凍結所有參數"""
    ...
    teacher = MambaPixelFormer(
        dinov2_model=args.teacher_model,
        min_depth=0.1,
    ...
    print(f"== Teacher loaded: {total_params:,} parameters (all frozen)")
    print("=" * 60)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # 載入 Teacher 模型（蒸餾用）
    # ===================================================================
    teacher = load_teacher(args, args.gpu if args.gpu is not None else 0)
    use_distill = teacher is not None
    if use_distill:
        print(f"== Distillation enabled: weight = {args.distill_weight}")
    else:
        print("== No teacher model, training without distillation")
    ...
    if args.backbone_type == "dinov2":
        ...
        optimizer = torch.optim.Adam([{'params': (model.module if hasattr(model, 'module') else model).parameters()}],
    ...
    # Loss
    # ===================================================================
    silog_criterion = SafeSILogLoss(variance_focus=args.variance_focus)
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            if args.dataset == 'nyu':
                ...
                mask = (depth_gt > 1.0) & (depth_gt < args.max_depth)
            ...
            if isinstance(depth_est, dict):
                ...
                pred_final = depth_est.clamp(min=1e-3, max=args.max_depth)

            # GT loss
            loss_gt = silog_criterion(pred_final, depth_gt, mask)

            # 蒸餾 loss
            if use_distill:
                with torch.no_grad():
                    teacher_depth = teacher(image)
                    if isinstance(teacher_depth, dict):
                        teacher_depth = teacher_depth['final']
                    teacher_depth = teacher_depth.clamp(min=1e-3, max=args.max_depth)
                
                loss_distill = F.l1_loss(pred_final[mask], teacher_depth[mask])
                loss = loss_gt + args.distill_weight * loss_distill
            else:
                loss = loss_gt
            ...
            if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
                if use_distill:
                    print('[epoch][s/s_per_e/gs]: [{}][{}/{}/{}], lr: {:.12f}, loss: {:.6f} (gt: {:.6f}, distill: {:.6f})'.format(
                        epoch, step, steps_per_epoch, global_step, current_lr, loss, loss_gt, loss_distill))
                else:
            ...
            if global_step and global_step % args.log_freq == 0 and not model_just_loaded:
                ...
                if not args.multiprocessing_distributed or (args.multiprocessing_distributed
                    ...
                    writer.add_scalar('gt_loss', loss_gt, global_step)
                    if use_distill:
                        writer.add_scalar('distill_loss', loss_distill, global_step)
