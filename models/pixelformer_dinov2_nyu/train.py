...
def _allreduce_small_cpu_sum(x_cpu: torch.Tensor):
    ...
    if backend == "nccl":
        g = _get_gloo_group()
        dist.all_reduce(x_cpu, op=dist.ReduceOp.SUM, group=g)  # 走 Gloo（TCP），不吃 /dev/shm
    else:
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # ===================================================================
    # 建立模型：根據 backbone_type 選擇 Swin 或 DINOv2
    # ===================================================================
    ...
    # ===================================================================
    # Optimizer：分離 backbone 和其他參數，使用不同學習率
    # ===================================================================
    if args.backbone_type == "dinov2":
        # DINOv2: backbone 用較小的學習率
        backbone_params = []
        other_params = []
        ...
        print(f"== Optimizer: backbone lr = {args.learning_rate * args.backbone_lr_factor:.2e}, other lr = {args.learning_rate:.2e}")
    else:
        # Swin: 原本的設定
    ...
    if args.checkpoint_path != '':
        if os.path.isfile(args.checkpoint_path):
            ...
            # 2. 檢查是否有 Optimizer 狀態
            if 'optimizer' in checkpoint:
                optimizer.load_state_dict(checkpoint['optimizer'])
                print("== Optimizer state loaded.")
            else:
                print("== Warning: No optimizer state found in checkpoint. Starting with a fresh optimizer.")

            # 3. 檢查是否有 global_step
            if not args.retrain and 'global_step' in checkpoint:
                global_step = checkpoint['global_step']
                print("== Resuming from global_step {}".format(global_step))
            else:
                print("== Warning: No global_step found. Starting from global_step 0.")
        else:
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            for i, param_group in enumerate(optimizer.param_groups):
                if args.backbone_type == "dinov2":
                    # DINOv2: 分別更新 backbone 和其他參數的學習率
                    if i == 0:  # backbone
